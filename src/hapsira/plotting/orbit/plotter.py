...
import hapsira.plotting.orbit.backends as orbit_plotter_backends
from hapsira.plotting.util import BODY_COLORS, generate_label
...
class Trajectory(
    namedtuple("Trajectory", ["coordinates", "position", "colors", "dashed", "label"])
):
...
class OrbitPlotter:
    ...
    def __init__(
        ...
        *,
        ...
        Parameters
        ----------
        backend : ~hapsira.plotting.orbit.backends._base.OrbitPlotterBackend
        ...
        plane : ~hapsira.frames.Plane, optional
    ...
    def backend(self):
        ...
        Returns
        -------
        ~hapsira.plotting.orbit.backends._base.OrbitPlotterBackend
    ...
    def set_body_frame(self, body, epoch=None):
        ...
        Parameters
        ----------
        body : hapsira.bodies.SolarSystemPlanet
            Body.
    ...
    def _add_trajectory(self, trajectory):
        ...
        if self.backend.is_2D:
            ...
            if position is not None:
                position = (
                    np.asarray(
                        self._project([position.to_value(self.length_scale_units)])
                    ).flatten()
    ...
    def plot_maneuver(
        ...
        Parameters
        ----------
        initial_orbit : ~hapsira.twobody.orbit.Orbit
            The base orbit for which the maneuver will be applied.
        maneuver : ~hapsira.maneuver.Maneuver
        ...
        *maneuver_phases, final_phase = initial_orbit.apply_maneuver(
        ...
        if self.backend.is_2D:
            final_phase_position = (
                np.asarray(
                    self._project([final_phase.r.to_value(self.length_scale_units)])
                ).flatten()
                * self.length_scale_units
            )
        else:
